...
- **核心模块**：
  ...
  - 文本选区通道：外壳 `frontend/src/style.css` 与预览 `tokens-*.css` 成对维护 `--selection-bg`（暗 `rgba(37, 99, 235, 0.45)` / 亮 `#bfdbfe`），统一 CodeMirror 选区层、外壳原生选区与预览 `::selection`；半透明字面量只登记在外壳 raw palette（`--blue-600-a45` / `--white-a04` / `--zinc-900-a04`），语义令牌层只做 `var()` 引用；编辑器活动行固定为约 4% 半透明的 `--editor-active-line`，不得回退为不透明表面色（CM6 选区层绘制在行元素背景之下，不透明活动行会遮住当前行选区）；聚焦选区规则带 `.cm-selectionLayer` 层级类以高于 oneDark 同权重规则；模式切回 Editor/Split 显式 `cm.requestMeasure()`；预览 `::selection` 强制 `color: var(--fg)`，避免深色代码块蓝底蓝字
...
- 「★ 编辑列在 Preview 模式为 `display:none`，切回 Editor/Split 后必须显式 `cm.requestMeasure()`——原因：CodeMirror 只通过异步 Intersection/ResizeObserver 感知可见性恢复（ResizeObserver 回调还带 75ms 静默判定），其间视口停留在 0x0 陈旧测量；无头/自动化在切换后派发选区前还须调用一次同步 `view.measure()`，且派发后要再调用一次——选区层重绘被排为 measure 请求、由动画帧排空，虚拟时间不保证触发（startup.check.mjs 修复前约 1/6 超时，派发前后各一次同步 measure 后 25/25 通过）」
- 「★ 聚焦选区规则必须带 `.cm-selectionLayer` 层级类（`:root[data-theme] .cm-editor.cm-focused .cm-selectionLayer .cm-selectionBackground`）——原因：oneDark 的 `&.cm-focused > .cm-scroller > .cm-selectionLayer .cm-selectionBackground` 与两层类简写同为 (0,5,0)，生效只能靠 esbuild 把外壳样式排在其后；补图层类后为 (0,6,0)，从规则结构上确定胜出」
