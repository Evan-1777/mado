...
- 「`baseName` 的正则是 `/[/\\/]/` 字面量，双反斜杠是正确写法——原因：正则字面量中 `\\` 匹配单个 `\`，与字符串字面量不同；TOC 修复（27d4a80）曾把它误改回单反斜杠，导致 Windows 路径 `C:\\dir\\file.md` 不再分割，标题栏显示整条路径（`C:/` 全角斜杠不受影响）」（后续全局排查双反斜杠时须跳过此正则）
